...
`s3connector.enabled` | Switch this to true if you have access to the s3connector Amp | `false`
`s3connector.config` | S3 Configuration https://docs.alfresco.com/s3connector/references/s3-contentstore-ref-config-props.html. Example: s3connector.config.bucketName: myS3Bucket | `{}`
`s3connector.secrets` | S3 Secrets Configuration https://docs.alfresco.com/s3connector/references/s3-contentstore-ref-config-props.html. Example: s3connector.secrets.accessKey: AJJJJJJJJ | `{}`
