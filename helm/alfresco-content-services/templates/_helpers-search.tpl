...
{{- with .Values }}
  ...
  {{- else if (index . "alfresco-search" "enabled") }}
    {{- if eq (index . "alfresco-repository" "configuration" "search" "flavor") "solr6" }}
      {{- print "solr6" }}
    {{- else }}
      {{ fail ".Values.alfresco-repository.configuration.search.flavor must be set to solr6" }}
    {{- end }}
  {{- else }}
    {{- print "noindex" }}
  {{- end }}
...
{{- $elasticsearch_audit_url := "" }}
  ...
  {{- else }}
    {{- with (index .Values "elasticsearch") }}
      {{- if .enabled }}
        {{- $elasticCtx := dict "Values" . "Chart" $.Chart "Release" $.Release }}
        {{- $auditEsHost := include "elastic.es-servicename" $elasticCtx }}
        {{- $auditEsPort := 9200 }}
        {{- $auditEsProto := "http" }}
