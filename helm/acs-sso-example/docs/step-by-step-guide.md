...
One of the goals is to make this chart as simple as possible while still
providing a way to deploy exactly the platform you want. So most of the kubernetes
...
component we want to use. This type of Helm chart is often referred to as an
*umbrella* chart.

Here we will see how to declare dependencies and configure them properly so they
work together.
...
> Note: there is neither a search component nor a transformation service.

This setup will be enriched with more components configured for SSO using
...
> Docker-desktop, Podman or minikube should be OK to use if properly configured.
...
often provided as part of the kubernetes desktop distributions but if you
don't have it [install it](https://helm.sh/docs/intro/install/).
...
So far we have just setup 2 independent components. They don't interact together.
...
Let's first create the secrets where the credentials will be available to
components which need them. To avoid spreading credentials where not needed,
we'll have one for each service:

- the database secret can be defined as follows in `templates/secret-db.yaml
